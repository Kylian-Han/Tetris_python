...
class Jeu:
	...
	def start(self):
		self._afficherTexte('Tetris', cons.CENTRE_FENETRE, font = 'titre')
		self._afficherTexte('Appuyer sur une touche...', cons.POS)
		self._attente()

	def stop(self):
		self._afficherTexte('Perdu', cons.CENTRE_FENETRE, font='titre')
		self._attente()
		self._quitter()
...
#		print("Position testée valide: x=%s, y=%s" % (x, y))
	...
	def _gererEvenements(self):
		...
		if event == K_p:
			...
			self._afficherTexte('Pause', cons.CENTRE_FENETRE, font='titre')
			self._afficherTexte('Appuyer sur une touche...', cons.POS)
			self._attente()
		elif event == K_LEFT:
	...
	def _dessinerPlateau(self):
		...
		if self.current is not None:
			for position in self.coordonnees:
				couleur = cons.COULEURS.get(self._getCurrentPieceColor())
				coordonnees = tuple([cons.START_PLATEAU[k] + position[k] * cons.TAILLE_BLOC[k] for k in range(2)])
				pygame.draw.rect(self.surface, couleur, coordonnees + cons.TAILLE_BLOC)
		self.score, self.pieces, self.lignes, self.tetris, self.niveau#TODO
		self._afficherTexte('Score: >%s' % self.score, cons.POSITION_SCORE)
		self._afficherTexte('Pièces: %s' % self.pieces, cons.POSITION_PIECES)
		self._afficherTexte('Lignes: %s' % self.lignes, cons.POSITION_LIGNES)
		self._afficherTexte('Tetris: %s' % self.tetris, cons.POSITION_TETRIS)
		self._afficherTexte('Niveau: %s' % self.niveau, cons.POSITION_NIVEAU)
