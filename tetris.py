...
class Jeu:
    ...
    def _afficher_texte(
        self, text : str, position : tuple[int, int], couleur : int = 9, font : str = 'defaut'
        ) -> None:
        """
          Affiche un texte sur la surface à une position donnée avec une couleur 
          et une police spécifiées.
    ...
    def _get_event(self):
        ...
        Retourne :
            int ou None : Code de la touche pressée lors d'un événement KEYDOWN, ou None si aucun 
            événement pertinent.
        """
    ...
    def _get_current_piece_color(self) -> int:
        ...
        Parcourt la matrice représentant la pièce courante et retourne la première valeur
        non nulle,
    ...
    def _est_valide(self, x : int=0, y:int=0, r:int=0) -> bool:
        """
        Vérifie si la position actuelle ou une position donnée 
        (avec décalage x, y et rotation r) de la pièce est valide sur le plateau de jeu.
        Args:
            x (int, optionnel): Décalage horizontal à appliquer à la position de la pièce. 
            Par défaut à 0.
            y (int, optionnel): Décalage vertical à appliquer à la position de la pièce. Par 
            défaut à 0.
            r (int, optionnel): Nombre de rotations à appliquer à la pièce. Par défaut à 0 
            (aucune rotation).
        Returns:
            bool: True si la position est valide (dans les limites du plateau et non occupée), 
            False sinon.
        """
...
#		print('Position testée valide: x=%s, y=%s' % (x, y))
    ...
    def _poser_piece(self) -> None:
        ...
        - Gère le cas particulier du 'Tetris' (quatre lignes complétées simultanément) et augmente 
        ...
        for i, line in enumerate(self.plateau[::-1]):
            for case in line:
                ...
                print(self.plateau)
                print(f'>>> {cons.DIM_PLATEAU[1]-1-i}')
    ...
    def _first(self) -> None:
        ...
        Cette méthode configure le plateau de jeu, réinitialise le score, le nombre 
        de pièces jouées,
        le nombre de lignes complétées, le nombre de tetris réalisés et le 
        niveau du joueur.
        Elle prépare également la prochaine pièce à jouer et indique que la 
    ...
    def _next(self) -> None:
        ...
        - Met à jour la pièce courante avec la prochaine pièce et génère une nouvelle 
        pièce suivante.
    ...
    def _gerer_evenemet(self) -> None:
        ...
        - Déplace la pièce vers la gauche, la droite ou le bas selon les flèches directionnelles, 
    ...
    def _gerer_gravite(self) -> None:
        ...
        Si le temps écoulé depuis la dernière chute dépasse la valeur de gravité définie, 
        la méthode tente de déplacer la pièce vers le bas :
        - Si la position actuelle est invalide, la pièce est remontée d'une ligne, ses 
        données sont recalculées et elle est posée sur le plateau.
        - Si la position actuelle est valide mais qu'elle ne le serait pas en descendant 
        d'une ligne, la pièce est posée sur le plateau.
    ...
    def _dessiner_plateau(self) -> None:
        ...
        pygame.draw.rect(
            ...
            self.surface, cons.COULEURS[8], cons.START_PLABORD+cons.TAILLE_PLABORD, cons.BORDURE_PLATEAU
            )
...
# pylint: disable=consider-using-generator
                coordonnees = tuple(
                    [cons.START_PLATEAU[k] + position[k] * cons.TAILLE_BLOC[k] for k in range(2)]
                    )
...
# pylint: disable=consider-using-generator
        ...
        self._afficher_texte(f'Score: >{self.score}', cons.POSITION_SCORE)
        self._afficher_texte(f'Pièces: {self.pieces}', cons.POSITION_PIECES)
        self._afficher_texte(f'Lignes: {self.lignes}', cons.POSITION_LIGNES)
        self._afficher_texte(f'Tetris: {self.tetris}', cons.POSITION_TETRIS)
        self._afficher_texte(f'Niveau: {self.niveau}', cons.POSITION_NIVEAU)
